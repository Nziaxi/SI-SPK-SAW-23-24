            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <!-- Form -->
                        <div class="col-xl">
                            <h4 class="fw-light">Edit Data Riwayat Karyawan</h4>
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <form
                                        action="{{ route('riwayat-penilaian.update', ['id' => $employee->id, 'year' => $year]) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-3">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-control" name="criteria[{{ $weight->id }}]">
                                                    @foreach ($subCriterias->where('criteria_id', $weight->criteria_id) as $sub)
                                                        <option value="{{ $sub->id }}"
                                                            {{ $histories->where('employee_id', $employee->id)->where('criteria_id', $weight->id)->where('year', $year)->pluck('sub_criteria_id')->contains($sub->id)? 'selected': '' }}>
                                                            {{ $sub->keterangan }}
                                                        </option>
                                                    @endforeach
                                                </select>
